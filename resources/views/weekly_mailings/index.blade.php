...
<head>
    ...
    <title>RMSys - Weekly Mailing</title>
    ...
    <style>
        ...
        @media print {
            ...
            #weeklymailingTable tbody tr:nth-child(even) {
                background-color: #f8f8f8 !important;
            }
            #weeklymailingTable tbody tr {
                background-color: #fff !important;
            }
        ...
        #weeklymailingTable tbody tr:last-child {
            border-bottom: none; /* No border for the last row */
        }

        #weeklymailingTable tbody td {
...
<body class="bg-gray-50 dark:bg-gray-900 dark:text-gray-200 font-sans antialiased">
        ...
        >
            ...
            <nav class="flex-1 px-3 py-4 space-y-1">
                <a href="{{ route('home') }}" class="sidebar-link block px-4 py-2 text-sm font-medium rounded-md flex items-center transition-colors duration-200
                    {{ Request::routeIs('home') ? 'bg-indigo-700 text-white shadow-md' : 'hover:bg-gray-700 hover:text-white' }}"
                ...
                <a href="{{ route('advisory.index') }}" class="sidebar-link block px-4 py-2 text-sm font-medium rounded-md flex items-center transition-colors duration-200
                    {{ Request::routeIs('advisory.index') ? 'bg-indigo-700 text-white shadow-md' : 'hover:bg-gray-700 hover:text-white' }}"
                    aria-current="{{ Request::routeIs('advisory.index') ? 'page' : 'false' }}">
                ...
                <a href="{{ route('memorandum.index') }}" class="sidebar-link block px-4 py-2 text-sm font-medium rounded-md flex items-center transition-colors duration-200
                    {{ Request::routeIs('memorandum.index') ? 'bg-indigo-700 text-white shadow-md' : 'hover:bg-gray-700 hover:text-white' }}"
                    aria-current="{{ Request::routeIs('memorandum.index') ? 'page' : 'false' }}">
                ...
                <a href="{{ route('specialorder.index') }}" class="sidebar-link block px-4 py-2 text-sm font-medium rounded-md flex items-center transition-colors duration-200
                    {{ Request::routeIs('specialorder.index') ? 'bg-indigo-700 text-white shadow-md' : 'hover:bg-gray-700 hover:text-white' }}"
                    aria-current="{{ Request::routeIs('specialorder.index') ? 'page' : 'false' }}">
                ...
                <a href="{{ route('weeklymailing.index') }}" class="sidebar-link block px-4 py-2 text-sm font-medium rounded-md flex items-center transition-colors duration-200
                    {{ Request::routeIs('weeklymailing.index') ? 'bg-indigo-700 text-white shadow-md' : 'hover:bg-gray-700 hover:text-white' }}"
                    aria-current="{{ Request::routeIs('weeklymailing.index') ? 'page' : 'false' }}">
        ...
        <main class="flex-1
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 md:p-8">
                <div class="flex flex-col gap-4 mb-6">
                    <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Weekly Mailing</h1>
                    <div class="flex flex-wrap items-center gap-3 justify-start w-full">
                        <div class="relative w-full md:w-auto flex-grow">
                            ...
                            <input
                                id="searchInput"
                                type="text"
                                placeholder="Search weekly mailings..."
                                ...
                                aria-label="Search weekly mailings"
                            />
                        </div>
                    ...
                    <div class="flex flex-wrap items-center gap-3 justify-end w-full">
                        <a
                            href="{{ route('weeklymailing.create') }}"
                            id="newWeeklyMailingButton"
                            ...
                            aria-label="Add New Weekly Mailing"
                            title="Add New Weekly Mailing"
                        >
                ...
                <div class="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                    <table
                        id="weeklymailingTable"
                        class="min-w-full text-sm text-left text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800"
                        role="table"
                        aria-label="Weekly Mailing Report"
                    >
                        <thead class="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 border-b border-gray-200 dark:border-gray-600">
                            <tr>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Service Division Unit</th>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Date Received</th>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Consignee</th>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Content</th>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Courier</th>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Tracking Number</th>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Date Shipped</th>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Code</th>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider text-center no-print">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                            @if(isset($weeklymailings) && count($weeklymailings) > 0)
                                @foreach($weeklymailings as $weeklymailing)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-850 transition-colors duration-150">
                                    <td class="px-4 py-3">{{ $weeklymailing->service_division_unit }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap">{{ \Carbon\Carbon::parse($weeklymailing->date_records_received)->format('Y-m-d') }}</td>
                                    <td class="px-4 py-3">{{ $weeklymailing->consignee }}</td>
                                    <td class="px-4 py-3">{{ $weeklymailing->content }}</td>
                                    <td class="px-4 py-3">{{ $weeklymailing->courier }}</td>
                                    <td class="px-4 py-3">{{ $weeklymailing->tracking_number }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap">{{ \Carbon\Carbon::parse($weeklymailing->date_shipped)->format('Y-m-d') }}</td>
                                    <td class="px-4 py-3">{{ $weeklymailing->code }}</td>
                                    <td class="px-4 py-3 text-center whitespace-nowrap no-print">
                                        <div class="inline-flex items-center space-x-2">
                                            <a href="{{ route('weeklymailing.edit', $weeklymailing->id) }}" title="Edit Weekly Mailing" class="text-yellow-600 hover:text-yellow-800 dark:text-yellow-400 dark:hover:text-yellow-300 transition-colors duration-200">
                                                <i class="fas fa-edit" aria-hidden="true"></i>
                                            </a>
                                            <form action="{{ route('weeklymailing.destroy', $weeklymailing->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this weekly mailing?');" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" title="Delete Weekly Mailing" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 transition-colors duration-200">
                            ...
                            @else
                                <tr>
                                    <td colspan="5" class="px-4 py-3 text-center text-gray-500 dark:text-gray-400">No weekly mailings found.</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
    ...
    <script>
        $(document).ready(function() {
            ...
            const toggleDarkMode = () => {
                ...
                if ($.fn.DataTable.isDataTable('#weeklymailingTable')) {
                    $('#weeklymailingTable').DataTable().draw();
                }
            };
        ...
        var table = $('#weeklymailingTable').DataTable({
            ...
            "language": {
                ...
                "searchPlaceholder": "Search weekly mailings...",
            ...
            ['10', '25', '50', 'All'] // Display text for each value
                ...
                buttons: [
                    // Excel Export Button
                    {
                        ...
                        title: 'Weekly Mailing Report - ' + new Date().toLocaleDateString('en-CA', { year: 'numeric', month: '2-digit', day: '2-digit' }), // Changed locale to 'en-CA' for YYYY-MM-DD
                        ...
                        filename: function() {
                            ...
                            return `Weekly_Mailing_Report_${year}${month}${day}`;
                        }
                    },
                    // PDF Export Button
                    {
                        ...
                        title: 'Weekly Mailing Report - ' + new Date().toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' }),
                        className: 'hidden-button', // Hide the default button
                        orientation: 'landscape', // Landscape orientation for more columns
                        pageSize: 'A4',
                        exportOptions: {
                            columns: ':visible:not(.no-print)' // Export only visible columns, exclude 'no-print' (actions)
                        },
                        customize: function (doc) {
                            ...
                            doc.content[doc.content.length - 1].alignment = 'center';
                        ...
                        const formattedDate = `${year}-${month}-${day}`; // yyyy-mm-dd format
                            ...
                            return `
                                <div class="print-header" style="text-align: center; margin-bottom: 20px; color: #000;">
                                    ...
                                    <p style="font-size: 16px; font-weight: bold; margin-bottom: 3px; line-height: 1.2;">WEEKLY MAILING REPORT</p>
    ...
    $.fn.dataTable.ext.search.push(
        function(settings, data, dataIndex) {
            ...
            // Get date values from the correct columns (0-indexed)
            // Column 1: Date Received
            var dateReceived = moment(data[1], 'YYYY-MM-DD'); 
            // Column 6: Date Shipped
            var dateShipped = moment(data[6], 'YYYY-MM-DD');

            var showRow = false;

            // Condition 1: No dates entered in filter, so show all rows.
            if (!min.isValid() && !max.isValid()) {
                showRow = true;
            } 
            // Condition 2: Filter by Date Received column (index 1)
            else if (dateReceived.isValid() && 
                    ((!min.isValid() || dateReceived.isSameOrAfter(min, 'day')) &&
                        (!max.isValid() || dateReceived.isSameOrBefore(max, 'day')))) {
                showRow = true;
            }
            // Condition 3: Filter by Date Shipped column (index 6) if Date Received didn't match
            else if (dateShipped.isValid() && 
                    ((!min.isValid() || dateShipped.isSameOrAfter(min, 'day')) &&
                        (!max.isValid() || dateShipped.isSameOrBefore(max, 'day')))) {
                showRow = true;
            }

            // If none of the above conditions met, hide the row
            return showRow;
        }
    );
    ...
    table.columns().every(function(index) {
        var column = this;
        // Skip the "Actions" column (index 8) if you don't want it toggled
        if (index === 8) { // Only skip "Actions" column for checkbox generation
            return;
        }
