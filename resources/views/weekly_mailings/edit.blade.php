...
<head>
    ...
    <title>RMSys - Edit Mailing #{{ $weeklymailing->id }}</title>

    <link rel="icon" href="https://i.ibb.co/whXMHLFg/Philippine-Statistics-Authority-svg.png" type="image/png">

    <script>
        // Check for theme in localStorage or system preference immediately (consistent)
    ...
    <script>
        // Tailwind CSS configuration (consistent)
        tailwind.config = {
            darkMode: 'class',
            theme: {
                extend: {
    ...
    <style>
        /* Custom scrollbar for better aesthetics (consistent) */
        ::-webkit-scrollbar {
            width: 8px;
            height: 8px;
        }
    ...
    >
        ...
        <nav class="flex-1 px-3 py-4 space-y-1">
            ...
            <a href="{{ route('weeklymailing.index') }}" class="sidebar-link block px-4 py-2 text-sm font-medium rounded-md flex items-center transition-colors duration-200
                bg-indigo-700 text-white shadow-md" aria-current="{{ Request::routeIs('weeklymailing.*') ? 'page' : 'false' }}"> {{-- Added shadow-md --}}
    ...
    <div class="flex flex-col flex-grow lg:ml-64 bg-gray-50 dark:bg-gray-900">
        <main class="flex-1 flex items-center justify-center p-6 md:p-8 text-gray-800 dark:text-gray-200">
            <div class="w-full max-w-3xl bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl mx-auto">
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white mb-6">Edit Weekly Mailing #{{ $weeklymailing->id }}</h1> {{-- Consistent title styling --}}
                ...
                <form method="POST" action="{{ route('weeklymailing.update', ['weeklymailing' => $weeklymailing]) }}">
                    @csrf
                    @method('PUT')

                    <div class="grid grid-cols-1 gap-6"> {{-- Consistent primary layout --}}
                        <div>
                            <label for="service_division_unit" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Service Division Unit</label>
                            <input type="text" id="service_division_unit" name="service_division_unit" value="{{ old('service_division_unit', $weeklymailing->service_division_unit) }}" required
                        ...
                        <div>
                            <label for="consignee" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Consignee</label>
                            <input type="text" id="consignee" name="consignee" value="{{ old('consignee', $weeklymailing->consignee) }}" required
                                class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                        </div>

                        <div>
                            <label for="content" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Content</label>
                            <textarea id="content" name="content" rows="3" required
                                class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">{{ old('content', $weeklymailing->content) }}</textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6"> {{-- Grouping date fields in a two-column grid --}}
                            <div>
                                <label for="date_records_received" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Date Records Received</label>
                                <input type="date" id="date_records_received" name="date_records_received" value="{{ old('date_records_received', $weeklymailing->date_records_received) }}" required
                                    class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                            </div>

                            <div>
                                <label for="date_shipped" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Date Shipped</label>
                                <input type="date" id="date_shipped" name="date_shipped" value="{{ old('date_shipped', $weeklymailing->date_shipped) }}" required
                                    class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                            </div>
                        </div>

                        <div>
                            <label for="courier" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Courier</label>
                            <input type="text" id="courier" name="courier" value="{{ old('courier', $weeklymailing->courier) }}" required
                                class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                        </div>

                        <div>
                            <label for="tracking_number" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Tracking Number</label>
                            <input type="text" id="tracking_number" name="tracking_number" value="{{ old('tracking_number', $weeklymailing->tracking_number) }}"
                                class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                        </div>

                         <div>
                            <label for="code" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Code</label>
                            <input type="text" id="code" name="code" value="{{ old('code', $weeklymailing->code) }}"
                                class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                        </div>
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Theme Toggle Functionality (Now unified across all edit pages)
        ...
        // Gmail Link functionality (consistent)
        ...
        // Removed searchInput logic as it's not relevant for an edit page and was inconsistent.
    });
</script>
