...
<head>
    ...
    <title>RMSys - Create New Weekly Mailing</title>
    ...
    >
        <div class="px-4 py-5 border-b border-gray-700 flex items-center justify-between">
            <a href="{{ route('home') }}" class="flex items-center hover:opacity-90 transition-opacity" aria-label="Go to Dashboard">
                ...
                >
                    <img
                        src="https://i.ibb.co/whXMHLFg/Philippine-Statistics-Authority-svg.png" {{-- Consistent logo source --}}
                        alt="RMSys Logo"
        ...
        <nav class="flex-1 px-3 py-4 space-y-1"> {{-- Consistent padding --}}
            ...
            <a href="{{ route('weeklymailing.index') }}" class="sidebar-link block px-4 py-2 text-sm font-medium rounded-md flex items-center transition-colors duration-200
                bg-indigo-700 text-white shadow-md" aria-current="{{ Request::routeIs('weeklymailing.*') ? 'page' : 'false' }}"> {{-- Added shadow-md for active link --}}
    ...
    <div class="flex flex-col flex-grow lg:ml-64 bg-gray-50 dark:bg-gray-900">
        <main class="flex-1 flex items-center justify-center p-6 md:p-8 text-gray-800 dark:text-gray-200">
            <div class="w-full max-w-3xl bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl mx-auto">
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white mb-6">Create New Weekly Mailing</h1> {{-- Consistent title styling --}}
                ...
                <form method="POST" action="{{ route('weeklymailing.store') }}">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 gap-6"> {{-- Consistent primary layout --}}
                        <div>
                            ...
                            <input
                                type="text"
                                id="service_division_unit"
                                name="service_division_unit"
                                value="{{ old('service_division_unit') }}"
                                placeholder="Service Division Unit"
                        ...
                        <div>
                            ...
                            <input
                                type="date"
                                id="date_records_received"
                                name="date_records_received"
                                value="{{ old('date_records_received') }}"
                                class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white"
                                required>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6"> {{-- Grouping related fields --}}
                            <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6"> {{-- Grouping remaining fields --}}
                            <div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Theme Toggle Functionality (Now unified across all pages)
        ...
        function updateDarkModeIcon() {
            ...
            if (desktopDarkModeToggle) {
                // Ensure desktopDarkModeToggle exists before trying to access its querySelector
        ...
        if (desktopDarkModeToggle) { // Ensure this button is correctly identified in HTML
