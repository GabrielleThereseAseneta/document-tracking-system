...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            #memorandumTable tbody tr:nth-child(even) {
                background-color: #f8f8f8 !important;
            }
            #memorandumTable tbody tr {
                background-color: #fff !important;
            }
        ...
        #memorandumTable tbody tr:last-child {
            border-bottom: none; /* No border for the last row */
        }

        #memorandumTable tbody td {
...
<body class="bg-gray-50 dark:bg-gray-900 dark:text-gray-200 font-sans antialiased">
        ...
        <main class="flex-1
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 md:p-8">
                <div class="flex flex-col gap-4 mb-6">
                    <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Memorandum</h1>
                    ...
                    <div class="flex flex-wrap items-center gap-3 justify-end w-full">
                        <a
                            href="{{ route('memorandum.create') }}"
                            id="newMemorandumButton"
                            ...
                            aria-label="Add New Memorandum"
                            title="Add New Memorandum"
                        >
                ...
                <div class="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                    <table
                        ...
                        role="table"
                        aria-label="Memorandum Report"
                    >
                        <thead class="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 border-b border-gray-200 dark:border-gray-600">
                            <tr>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Memorandum Number</th>
                                ...
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Memorandum Date</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                            @if(isset($memorandums) && count($memorandums) > 0)
                                @foreach($memorandums as $memorandum)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-850 transition-colors duration-150">
                                    <td class="px-4 py-3">{{ $memorandum->memo_number }}</td>
                                    <td class="px-4 py-3">{{ $memorandum->name }}</td>
                                    <td class="px-4 py-3">{{ $memorandum->title_description }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap">{{ \Carbon\Carbon::parse($memorandum->memo_date)->format('Y-m-d') }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap">{{ \Carbon\Carbon::parse($memorandum->date_received)->format('Y-m-d') }}</td>
                                    <td class="px-4 py-3 text-center whitespace-nowrap no-print">
                                        <div class="inline-flex items-center space-x-2">
                                            <a href="{{ route('memorandum.index', $memorandum->id) }}" title="View Memorandum" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 transition-colors duration-200">
                                            <a href="{{ route('memorandum.edit', $memorandum->id) }}" title="Edit Memorandum" class="text-yellow-600 hover:text-yellow-800 dark:text-yellow-400 dark:hover:text-yellow-300 transition-colors duration-200">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('memorandum.destroy', $memorandum->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this memorandum?');" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" title="Delete Memorandum" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 transition-colors duration-200">
                            ...
                            @else
                                <tr>
                                    <td colspan="5" class="px-4 py-3 text-center text-gray-500 dark:text-gray-400">No memorandums found.</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
    ...
    <script>
        $(document).ready(function() {
            ...
            const toggleDarkMode = () => {
                ...
                if ($.fn.DataTable.isDataTable('#memorandumTable')) {
                    $('#memorandumTable').DataTable().draw();
                }
            };
        ...
        var table = $('#memorandumTable').DataTable({
            ...
            ['10', '25', '50', 'All'] // Display text for each value
                ...
                buttons: [
                    // Excel Export Button
                    {
                        ...
                        title: 'Memorandum Report - ' + new Date().toLocaleDateString('en-CA', { year: 'numeric', month: '2-digit', day: '2-digit' }), // Changed locale to 'en-CA' for YYYY-MM-DD
                        ...
                        filename: function() {
                            ...
                            return `Memorandum_Report_${year}${month}${day}`;
                        }
                    },
                    // PDF Export Button
                    {
                        ...
                        title: 'Memorandum Report - ' + new Date().toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' }),
                        className: 'hidden-button', // Hide the default button
                        ...
                        const formattedDate = `${year}-${month}-${day}`; // yyyy-mm-dd format
                            ...
                            return `
                                <div class="print-header" style="text-align: center; margin-bottom: 20px; color: #000;">
                                    ...
                                    <p style="font-size: 16px; font-weight: bold; margin-bottom: 3px; line-height: 1.2;">MEMORANDUM REPORT</p>
    ...
    $.fn.dataTable.ext.search.push(
        function(settings, data, dataIndex) {
            var min = $('#minDate').val();
            var max = $('#maxDate').val();
            // Column index 2: Date of Memorandum, Column index 3: Date Received
            ...
            // var dateMemorandum = moment(data[2], 'MM/DD/YYYY').format('YYYY-MM-DD'); // Example if table date is MM/DD/YYYY
            // var dateMemorandum = moment(data[3], 'MM/DD/YYYY').format('YYYY-MM-DD'); // Example if table date is MM/DD/YYYY
            var dateMemorandum = data[2];
            ...
            var MemorandumMoment = moment(dateMemorandum);
            ...
            // Check if both memorandum and received dates fall within the range
            var memorandumInRange = (!minMoment || memorandumMoment.isSameOrAfter(minMoment, 'day')) &&
                                  (!maxMoment || memorandumMoment.isSameOrBefore(maxMoment, 'day'));
            ...
            // Return true if BOTH memorandum and received dates are in range
            return memorandumInRange && receivedInRange;
        }
    );
