...
<head>
    ...
    <title>RMSys - Edit Advisory #{{ $advisory->id }}</title>
    ...
    >
        ...
        <nav class="flex-1 px-3 py-4 space-y-1">
            ...
            <a href="{{ route('advisory.index') }}" class="sidebar-link block px-4 py-2 text-sm font-medium rounded-md flex items-center transition-colors duration-200
                bg-indigo-700 text-white shadow-md" aria-current="{{ Request::routeIs('advisory.*') ? 'page' : 'false' }}">
    ...
    <div class="flex flex-col flex-grow lg:ml-64 bg-gray-50 dark:bg-gray-900">
        <main class="flex-1 flex items-center justify-center p-6 md:p-8 text-gray-800 dark:text-gray-200">
            <div class="w-full max-w-3xl bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl mx-auto">
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white mb-6">Edit Advisory #{{ $advisory->id }}</h1>
                ...
                <form method="POST" action="{{ route('advisory.update', ['advisory' => $advisory]) }}">
                    @csrf
                    @method('PUT')

                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Issued By</label>
                            <input type="text" id="name" name="name" value="{{ old('name', $advisory->name) }}" required
                        ...
                        <div>
                            ...
                            <input type="text" id="title_description" name="title_description" value="{{ old('title_description', $advisory->title_description) }}" required
                                class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label for="date_of_advisory" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Date of Advisory</label>
                                <input type="date" id="date_of_advisory" name="date_of_advisory" value="{{ old('date_of_advisory', $advisory->date_of_advisory) }}" required
                                    class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                            </div>

                            <div>
                                <label for="date_received" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Date Received</label>
                                <input type="date" id="date_received" name="date_received" value="{{ old('date_received', $advisory->date_received) }}" required
                                    class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-8 flex justify-end space-x-3">
                        <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm px-4 py-2 rounded-md shadow-md inline-flex items-center transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            <i class="fas fa-save mr-2"></i> <span>Update</span>
                        </button>
                        <button type="button" onclick="history.back()" class="bg-gray-300 hover:bg-gray-400 text-gray-700 text-sm px-4 py-2 rounded-md shadow-md inline-flex items-center transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300">
