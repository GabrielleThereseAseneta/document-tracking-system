...
<head>
    ...
    <title>RMSys - Edit Special Order #{{ $specialorder->id }}</title>
    ...
    >
        ...
        <nav class="flex-1 px-3 py-4 space-y-1">
            ...
            <a href="{{ route('specialorder.index') }}" class="sidebar-link block px-4 py-2 text-sm font-medium rounded-md flex items-center transition-colors duration-200
                bg-indigo-700 text-white shadow-md" aria-current="{{ Request::routeIs('specialorder.*') ? 'page' : 'false' }}"> {{-- Added shadow-md --}}
    ...
    <div class="flex flex-col flex-grow lg:ml-64 bg-gray-50 dark:bg-gray-900">
        <main class="flex-1 flex items-center justify-center p-6 md:p-8 text-gray-800 dark:text-gray-200">
            <div class="w-full max-w-3xl bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl mx-auto">
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white mb-6">Edit Special Order #{{ $specialorder->id }}</h1> {{-- Consistent title styling --}}
                ...
                <form method="POST" action="{{ route('specialorder.update', ['specialorder' => $specialorder]) }}">
                    @csrf
                    @method('PUT')

                    <div class="grid grid-cols-1 gap-6"> {{-- Consistent primary layout --}}
                        <div>
                            <label for="so_number" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Special Order Number</label>
                            <input type="text" id="so_number" name="so_number" value="{{ old('so_number', $specialorder->so_number) }}" required
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $specialorder->name) }}" required
                                class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="title_description" name="title_description" value="{{ old('title_description', $specialorder->title_description) }}" required
                                class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6"> {{-- Dates in a two-column grid --}}
                            <div>
                                <label for="date_of_so" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Date of Special Order</label>
                                <input type="date" id="date_of_so" name="date_of_so" value="{{ old('date_of_so', $specialorder->date_of_so) }}" required
                                    class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                            </div>

                            <div>
                                <label for="date_received" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Date Received</label>
                                <input type="date" id="date_received" name="date_received" value="{{ old('date_received', $specialorder->date_received) }}" required
                                    class="w-full px-5 py-2.5 text-sm rounded-md border dark:border-gray-600 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                            </div>
                        </div>
                    </div>
