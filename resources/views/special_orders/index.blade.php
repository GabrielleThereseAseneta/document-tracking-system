...
<head>
    ...
    <title>RMSys - Special Order</title>
    ...
    <style>
        ...
        @media print {
            ...
            #specialorderTable tbody tr:nth-child(even) {
                background-color: #f8f8f8 !important;
            }
            #specialorderTable tbody tr {
                background-color: #fff !important;
            }
        ...
        #specialorderTable tbody tr:last-child {
            border-bottom: none; /* No border for the last row */
        }

        #specialorderTable tbody td {
...
<body class="bg-gray-50 dark:bg-gray-900 dark:text-gray-200 font-sans antialiased">
        ...
        <main class="flex-1
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 md:p-8">
                <div class="flex flex-col gap-4 mb-6">
                    <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Special Order</h1>
                    ...
                    <div class="flex flex-wrap items-center gap-3 justify-end w-full">
                        <a
                            href="{{ route('specialorder.create') }}"
                            id="newSpecialOrderButton"
                            ...
                            aria-label="Add New Special Order"
                            title="Add New Special Order"
                        >
                ...
                <div class="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                    <table
                        id="specialorderTable"
                        class="min-w-full text-sm text-left text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800"
                        role="table"
                        aria-label="Special Order Report"
                    >
                        <thead class="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 border-b border-gray-200 dark:border-gray-600">
                            <tr>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Special Order Number</th>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Name</th>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Title / Description</th>
                                <th scope="col" class="px-4 py-3 font-semibold text-xs uppercase tracking-wider">Date of Special Order</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                            @if(isset($specialorders) && count($specialorders) > 0)
                                @foreach($specialorders as $specialorder)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-850 transition-colors duration-150">
                                    <td class="px-4 py-3">{{ $specialorder->so_number }}</td>
                                    <td class="px-4 py-3">{{ $specialorder->name }}</td>
                                    <td class="px-4 py-3">{{ $specialorder->title_description }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap">{{ \Carbon\Carbon::parse($specialorder->date_of_so)->format('Y-m-d') }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap">{{ \Carbon\Carbon::parse($specialorder->date_received)->format('Y-m-d') }}</td>
                                    <td class="px-4 py-3 text-center whitespace-nowrap no-print">
                                        <div class="inline-flex items-center space-x-2">
                                            <a href="{{ route('specialorder.index', $specialorder->id) }}" title="View Special Order" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 transition-colors duration-200">
                                            <a href="{{ route('specialorder.edit', $specialorder->id) }}" title="Edit Special Order" class="text-yellow-600 hover:text-yellow-800 dark:text-yellow-400 dark:hover:text-yellow-300 transition-colors duration-200">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('specialorder.destroy', $specialorder->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this special order?');" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" title="Delete Special Order" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 transition-colors duration-200">
                            ...
                            @else
                                <tr>
                                    <td colspan="5" class="px-4 py-3 text-center text-gray-500 dark:text-gray-400">No special orders found.</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
    ...
    <script>
        $(document).ready(function() {
            ...
            const toggleDarkMode = () => {
                ...
                if ($.fn.DataTable.isDataTable('#specialorderTable')) {
                    $('#specialorderTable').DataTable().draw();
                }
            };
        ...
        var table = $('#specialorderTable').DataTable({
            ...
            "language": {
                ...
                "searchPlaceholder": "Search special orders...",
            ...
            ['10', '25', '50', 'All'] // Display text for each value
                ...
                buttons: [
                    // Excel Export Button
                    {
                        ...
                        title: 'Special Order Report - ' + new Date().toLocaleDateString('en-CA', { year: 'numeric', month: '2-digit', day: '2-digit' }), // Changed locale to 'en-CA' for YYYY-MM-DD
                        ...
                        filename: function() {
                            ...
                            return `Special_Order_Report_${year}${month}${day}`;
                        }
                    },
                    // PDF Export Button
                    {
                        ...
                        title: 'Special Order Report - ' + new Date().toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' }),
                        className: 'hidden-button', // Hide the default button
                        ...
                        const formattedDate = `${year}-${month}-${day}`; // yyyy-mm-dd format
                            ...
                            return `
                                <div class="print-header" style="text-align: center; margin-bottom: 20px; color: #000;">
                                    ...
                                    <p style="font-size: 16px; font-weight: bold; margin-bottom: 3px; line-height: 1.2;">SPECIAL ORDER REPORT</p>
    ...
    $.fn.dataTable.ext.search.push(
        function(settings, data, dataIndex) {
            var min = $('#minDate').val();
            var max = $('#maxDate').val();
            // Column index 2: Date of Special Order, Column index 3: Date Received
            ...
            // var dateSpecialOrder = moment(data[2], 'MM/DD/YYYY').format('YYYY-MM-DD'); // Example if table date is MM/DD/YYYY
            // var dateSpecialOrder = moment(data[3], 'MM/DD/YYYY').format('YYYY-MM-DD'); // Example if table date is MM/DD/YYYY
            var dateSpecialOrder = data[2];
            ...
            var SpecialOrderMoment = moment(dateSpecialOrder);
            ...
            // Check if both special order and received dates fall within the range
            var specialorderInRange = (!minMoment || specialorderMoment.isSameOrAfter(minMoment, 'day')) &&
                                  (!maxMoment || specialorderMoment.isSameOrBefore(maxMoment, 'day'));
            ...
            // Return true if BOTH special order and received dates are in range
            return specialorderInRange && receivedInRange;
        }
    );
    ...
    table.columns().every(function(index) {
        var column = this;
        // Skip the "Actions" column (index 5) if you don't want it toggled
        if (index === 5) { // Only skip "Actions" column for checkbox generation
            return;
        }
